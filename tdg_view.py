...
def draw_ui():
    """
    Draws UI elements (selected tower, wave, coins, towers placed, available/locked towers).
    When the game is over, displays final stats.
    """
    font = pygame.font.SysFont(None, 24)
    data = game.get_game_data()
    if data["game_over"]:
        screen.blit(font.render("GAME OVER", True, (255, 0, 0)), (10, rows * cell_size + 10))
...
def process_key(event):
    """
    Processes key events mapping keys to game actions.
    """
    ...
    if event.type == pygame.KEYDOWN and event.unicode in '123':
        ...
        if event.key == pygame.K_w:
            ...
            if not game.get_game_data()["game_started"]:
                action = 'START_WAVE'
                # Adjust enemy positions after starting wave.
